...
def main():
    ...
    while(client.isApiControlEnabled()):
        ...
        task = asyncio.gather(CustomVisionService.ClassifyImage(account_name,account_key,os.path.normpath(filepath + '.png'),inspectionID,connection_string,filepath,filename,"Sunny", 25)) #send image to custom vision for object detection
        tasks.append(task)
        time.sleep(2)
        count += 1
    ...
    predictionObject['Weather'] = 'Sunny'
    predictionObject['WindSpeed'] = 25
